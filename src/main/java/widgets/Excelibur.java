...
import widgets.table.MultiLineTableHeaderRenderer;
...
public class Excelibur extends JPanel {
	...
	public Excelibur(App owner) {
		...
		checkBoxUseHyperlinkIfAvailable.setToolTipText("Replaces cell content with the hyperlink if available");
		...
		infoPanel.add(new JLabel("Output (sub-)folder naming rule."));
		...
		reloadButton.setToolTipText(
				"Reimport the selected files. Be sure to have all imported Excel files closed or they won't be imported as Excel blocks the files when opened.");
	...
	private void importData() {
		...
		MultiLineTableHeaderRenderer headerCellRenderer = new MultiLineTableHeaderRenderer();
	...
	private boolean exportData(String outputFolder, String fileName) {
		translationMgr.setFlag(TranslationMgrFlags.Export.CONCAT_COMPONENT_AND_KEY,
				checkBoxMergeCompAndKey.isSelected());
		translationMgr.setFlag(TranslationMgrFlags.Export.DONT_EXPORT_EMPTY_VALUES,
